...
这是 Redux 用于其 reducer 的技术。 reducer 的签名是 (previousState, action) => newState，这意味着它将状态和操作作为参数并返回新状态作为结果。 更具体地说，reducer 不应该简单地更改 previousState 参数，它必须保持不变（我们将在 [Chapter 10] 中了解更多信息（https://subscription.packtpub.com/book/web_development/9781839213069/1 ), *确保纯度 – 不变性*)。
