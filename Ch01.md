...
OOP 一直是解决编写大型程序和系统的固有复杂性，以及开发简洁、可扩展、可伸缩的应用架构的一种方法；然而，由于当今网络应用的规模，所有代码库的复杂性都在不断增加。此外，JavaScript 的更新功能使得开发几年前还不可能实现的应用程序成为可能；例如，使用 Ionic、Apache Cordova 或 React Native 开发的移动（混合）应用程序，或者使用 Electron、Tauri 或 NW.js 开发的桌面应用程序。JavaScript 还通过 Node.js 或 Deno 迁移到了后端，因此今天，JavaScript 语言的使用范围已经大幅扩大，可以应对 现代设计所增加的所有复杂性。
